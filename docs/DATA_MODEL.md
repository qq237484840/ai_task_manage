...
> **变更预告（v0.9.0，2026-09-08）**：内容级判定重构（ADR-010/PD-018~024，`CR-002`/`ACR-002` Open）将调整判定基准语义 —— DATA-001 题目可保留可选字段但**不再作为判定基准**（端到端直判）、DATA-004/005/006 承载逐题对齐判定与"大模型综合评判（草稿→家长复核定稿）"结果；真实三方默认（ADR-011）出域面登记于 DATA-009/RISK-009。字段级同步在 CR/ACR 批准后统一执行（防二次漂移，O-8）。

## 核心实体

| DATA ID | 实体 | Owner 模块 | 数据库/存储 | Writer | Reader | 敏感级别 | 生命周期 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| DATA-001 | 作业任务（题目集/客观题参考答案，ADR-006） | M001 | SQLite | M001 | M002, M004, M005, M007 | 中（含学生作答对象） | 长期（可为 V2 画像留数据） |
| DATA-002 | 用户档案（家庭账号 + 学生档案，ADR-005） | M001（家庭空间/登录基础随 M001 落地） | SQLite | M001 | 认证/各模块 | 高（未成年人个人信息） | 长期 |
...
| 实体 | 关键属性（草案） |
| --- | --- |
| DATA-001 作业任务 | 任务ID/家庭ID/标题/学科/年级/题目集合(序号/题干/学科/题型[主观|客观]/可选辅助字段)/状态/发布时间/截止时间/关联学生档案 —— **判定基准=内容级端到端直判，参考答案字段不再是判定基准（ADR-010/CR-002）** |
| DATA-002 用户档案 | 家庭账号(账号ID/登录凭证哈希/显示名) + 学生档案(档案ID/家庭ID/姓名/年级/与账号关系/关联学校 school_id→schools[必填, ADR-008]) |
