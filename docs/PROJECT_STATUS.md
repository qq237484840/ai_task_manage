...
`v0.12.0`（**前端技术栈切换立项 + M002 编码并行**：用户技术方案变更 → **ADR-012 Accepted**（Vue3 + Vite + TypeScript + Vant 4，PD-025）+ **CHANGE-002 立项（Executing）** + **Task-003 签发（AGENT-M001）**（M001 页面迁移 + FastAPI 托管 `dist/`）；Task-002（AGENT-M002）后端编码并行、前端 UI 段后移接轨新栈）
...
- **前端技术栈切换（2026-09-08，ADR-012/CHANGE-002/Task-003）**：前端统一 **Vue3 + Vite + TypeScript + Vant 4** 工程（零构建原生 H5 退役，FastAPI 托管 `dist/`，REST 契约零影响）；AGENT-M001 等价迁移 M001 现有 6 视图 + 托管切换；Task-002 前端基础 UI 后移接轨（后端并行不受阻）
- 跨模块开放项：见下方"跨模块开放项"（v0.9.0 精校 + v0.11.0 关闭 O-8；**v0.12.0 关闭 O-6** = 前端统一工程承载）
...
7. **进行中（当前）**：**M002 编码（Task-002，AGENT-M002）**：实现 M002 v0.3.0（backend `modules/m002` + 图片存储链路 + tests + 九件套回填）——**前端基础 UI 段后移接轨**：先并行完成 **Task-003 前端技术栈切换（ADR-012/CHANGE-002，AGENT-M001：Vite+Vue3+TS+Vant 4 工程 + M001 页面迁移 + FastAPI 托管 `dist/`）**，再按新栈实现 M002 前端页面 → **Task-002 按 M002 模块级 DoD 独立收口（定稿 v0.3.0 Stable）** → 域 **M-A 全量验收**（含布置单拍照识别 + 真实三方 AI 模式）随 **M003** 到位后统一执行 → M-B~M-D 依序 → 全系统回归（真实三方 + Mock 双跑）
