...
`v0.10.0`（**M002 契约批准冻结**：用户批准 M002 契约草案 v0.3.0（2026-09-08）→ **Frozen** + Task-002 签发（AGENT-M002 Active）；**M001 合并 CHANGE 执行启动**（AGENT-M001））

## 当前阶段

**Phase 3 —— M002 开发 + M001 变更执行（2026-09-08）**：M002 契约 **v0.3.0 Frozen**（用户批准签署区）→ Task-002 已签发（AGENT-M002 Active，编码中）；M001 冻结面合并 CHANGE（`CR-001`/`CR-002`/`ACR-001`/`ACR-002`）由 AGENT-M001 执行中；主线计划、验收里程碑按功能域 A~D 重排（`ROADMAP.md`）
...
- V1 需求基线：REQ-001~009 全部 **Approved**（2026-09-08，含增补 REQ-009 学校基础资料，ADR-008）；REQ-001/009 随 M001 验收转 Done（REQ-001 受 CR-001/CR-002 复开）
- **M001（作业任务管理）= Stable**：契约 v0.1.1 Frozen（用户批准）+ Task-001 实现（backend/frontend/tests，54 passed）经 PM DoD 验收 APPROVED；**冻结面变更请求 `CR-001`（登记单容器）/`CR-002`（内容级判定链数据面）/`ACR-001`（两级主体认证）/`ACR-002`（判定与 Provider 架构重构）均 Approved（2026-09-08）**，待合并 CHANGE 执行（ADR-010/011 → Accepted，ADR-006/007 → Superseded 生效）
- **M002（作业图片采集与归属）契约 v0.3.0 = Frozen**（2026-09-08 用户批准）：先采后认归属保留、完成程度以内容级判定为准；**Task-002 已签发（AGENT-M002 Active），编码中**（依赖 M001 CHANGE 落地 `group_no`/两级主体接口）
- **M001 变更执行中（AGENT-M001，合并 CHANGE）**：`CR-001`（登记单容器化 + `group_no`）/`ACR-001`（两级主体）/`CR-002`/`ACR-002`（判定基准与 Provider 配置，内容级）→ 完成后回填 M001 契约与测试回归
- 跨模块开放项：见下方"跨模块开放项"（v0.9.0 精校：O-3/O-4 随 ADR-010 关闭口径更新）

## 已完成模块

| Module ID | 名称 | 状态 |
| --- | --- | --- |
| M001 | 作业任务管理 | Stable（v0.7.0 PM DoD 验收 APPROVED；冻结面变更 CR-001/CR-002/ACR-001/ACR-002 已 Approved，待合并 CHANGE 执行） |
...
| O-6 | 两级主体前端体验（学生登录/会话切换、家长代传路径、兜底入口） | ACR-001 批准后 | ADR-009 |
| O-7 | REQ-004~008 详情页"待确认点"残留精校（与各自模块契约轮同步） | 各模块契约轮（REQ-001~007 随 CR-002 精校内容级口径） | — |
| O-8 | `DATA_MODEL.md`/`INDEX.md` 字段级同步统一执行 | **已批准（CR-001/ACR-001/CR-002/ACR-002 Approved）** → M001 合并 CHANGE 执行时同步执行 | 防二次漂移 |
...
6. **进行中（当前）**：**M001 合并 CHANGE 执行**（CR-001/CR-002/ACR-001/ACR-002，AGENT-M001）→ 代码落地 + 测试回归 + 回填 M001 契约 → M002 编码（Task-002，依赖 CHANGE 的 group_no/两级主体）→ 依 ROADMAP 里程碑 M-A~M-D 验收推进（M001 变更 + M002 采集 = M-A 学生自主采集闭环）→ M003…M007 依序 → 全系统回归（真实三方 + Mock 双跑）
