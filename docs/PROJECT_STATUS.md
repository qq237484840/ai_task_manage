...
`v0.24.0`（**`CR-005` 落地并经 8 条验收全通过 → `Applied`（2026-09-16）：布置单「图片源」解析通路打通（链路 T 图片源 → Vision）+ 任务「重新解析」端点** —— ① **契约**：M001 增 `API-M001-022`（`POST /tasks/{task_id}/reparse`，状态 `Active`）+ M001/M002 内部服务接口各 +1（沿用**既有唯一回调通道**，非直连）；② **跨模块实现**：M002 `provide_task_source_image`（归属校验 + `ImageStore` 受控路径 + 只读；越权/查无/文件缺失 → `None`）+ `ensure_task_spec_image_provider_registered`（幂等 + 启动期自愈）；M001 `image_provider` 回调槽 + `task_parser._vision_is_real()` **防伪造硬约束**（**仅真实 Vision 才转发图片源** —— `BUG-004` 教训固化为判据）+ `task_service.reparse`（`placeholder` 写入 / `parsed` **整体替换** / 失败**不清空** / `confirmed` → `409 spec_confirmed`）；前端「重新解析」按钮（`confirmed` 隐藏、单请求 60s、反馈**以内容项为准不谎报**）；③ **真机证据（真实 Vision，8010）**：布置单图片源 `POST /tasks → 201 / spec_status=parsed / 4 项内容`（`math | 练习册 P23 第1-10题` 等）+ DATA-009 `task_spec_parse / qwen3.8-flash / mock=0 / status=ok / 16223ms`；跨家庭引用 → `201` 但 `placeholder / 0`（**不转发 = 不越权取图**）；`confirmed` → `409`；④ **验收（`Task-021`，8 条全通过）**：防伪造**红→绿**（移除判据 → `assert 'parsed' == 'placeholder'` 失败 + 审计 `task_reparsed parsed=True replaced=True` = 伪造草稿复现，`RED_EXIT=1`）、全量回归 **267 / 0 / 0 / 0**（基线 265 + 新增 2）、前端 `TSC_EXIT=0` / `BUILD_EXIT=0`、浏览器级**成功分支 7/7（真实 Vision）+ 失败分支 7/7**（经 `provider_mode=mock` 构造：toast「AI 未返回解析结果…」+ 任务未被伪造）；⑤ **新增用例**：`tests/integration/test_cr005_image_source_service.py`（服务级**真实装配**：跨家庭 → 观测桩 `sources` 无 `image`；本家 → 转发且 `image.path` 在受控根内）；⑥ **登记**：**`TD-006`**（M001 不校验图片源 `photo_id` 归属 —— 防越权仅由 M002 一道承担；当前**无越权读取路径** → 风险**低**）；⑦ **执行方式限制**：`Task-018`~`Task-021` 均由 **PM 代执行**（本机 IDE 无具备写权限的执行 subagent）→ **不构成独立第三方验收**（已如实标注各任务书 §5）；⑧ **过程失误如实记录**：构造失败分支时误将 `.env` 的 `AT_AI_*`（**含三方密钥**）打印到对话（`.env` 已 gitignore、未入库）→ **建议轮换中转密钥**（详见 `Task-021 §5` ⑦））

- **`v0.23.0` 基线**（**真实三方 AI 联调打通 + `BUG-005`/`BUG-006` 登记并修复收口（`Task-015`/`Task-016`，PM 代执行）**：① **真实 Provider 首次真实出网并产出正确业务结果** —— LLM `glm-5.3` 解析出 `math`+`chinese`（`task_spec_parse / mock=0 / status=ok`）、Vision `qwen3.8-flash` 对真实作业图给出 `links=[{subject:'math', source:'ai', confidence:1.0}]`（`photo_link_suggest / mock=0 / status=ok`）；残余风险 = 外部中转可用性波动（曾全站 502 / 超时，重试成功）；② **`BUG-005`（中）修复**：三方 403「余额/配额不足」原被误报「鉴权失败」且响应体被丢弃 → 新增 `AIErrorCode.QUOTA_EXHAUSTED`（不可重试）+ 403 配额线索精细化 + 三方摘要**脱敏 + ≤200 字符**入 `message`（`Task-015`，`AGENT-AI`）；③ **`BUG-006`（高）修复**：M001 `default_parser` 在 `real` + 禁兜底下仍静默 Mock 兜底（`spec_status=parsed`「假成功」）→ 新增 `_mock_fallback_allowed()`（显式 `mock` 模式放行；否则取 `allow_mock_fallback`；配置不可读保守放行 + warning），禁用时返回 `None` → 保持 `placeholder`（`Task-016`，`AGENT-M001`）；④ **验收证据**：全量回归 **251 / 0 failed / 0 errors / 0 skipped**（基线 237 + 新增 14 例）、两处**红→绿**（含 **API 面**：禁兜底 → `placeholder`，回退 → `parsed` 复现，`RED_EXIT=1`）、写区双证 + `read_lints=0`；⑤ **行为面变更（Release Note）**：`real` + `allow_mock_fallback=false` 时 AI 失败任务的 `spec_status` 由 `parsed` → **`placeholder`**（配置意图，无表结构/API 结构变更）；⑥ **执行方式限制**：本机 IDE 无具备写权限的执行 subagent（仅只读 `code-explorer`）→ 两任务由 **PM 代执行**，**不构成独立第三方复核**（已在 `Task-015 §5` / `Task-016 §5` 如实标注）；⑦ 另含 `Task-016 §3.6` **写区最小扩权**（`test_scenario6_*` 仅后移 env 切换时机、断言逐字不变））

- **`v0.22.0` 基线**（**④ 验收收口 + `CHANGE-003` 关闭**：`Task-014`（AGENT-M002）**去替身复审**交付 → **PM 独立复现全部成立** —— ① **判别力（PM 亲手）**：`AT_AI_PROVIDER_MODE=real` + `AT_AI_ALLOW_MOCK_FALLBACK=false` 下两例边界用例 **`FF`（实测 `status='unassigned'` + `suggestions=[]`，断言立败，`RED_EXIT=1`）** → 还原默认 `auto` 后 **`....`（4 passed，`GREEN_EXIT=0`）= 真红真绿**；② **全量回归（PM 实测）**：`pytest --tb=no -q --junitxml` → **`tests=237 failures=0 errors=0 skipped=0`、`EXIT=0`**（与基线上限一致，未删减用例）；③ **浏览器级剧本 5**：`.e2e/browser_evidence.json` = **18 项全 `true` / 0 项 `false`**（3 张照片经真实 AI 通路产出建议 → UI 采纳入口 → 逐张采纳 6 次全 200 → 门控满足 → 生成草稿 `model:"mock-vision"` → 家长确认）；④ **写区双证**：两处声称改动文件 `git status --porcelain` 均为 **`??`（未跟踪写区）**、Forbidden 区 mtime **全部早于写区起点**（`mock.py` 20:15:47 / `task_parser.py` 20:11:26 / `BUG-003.md` 20:23:19 等）；⑤ **`TD-003` 死代码清理**：`list_groups` 的 `except TypeError` 垫片已删（改按 `M001 v0.2.0 Frozen` 契约直调），真机集成 10 例 + 上层 40+ 例 0 触发；⑥ 同文件 `getattr(...,None)+raise` **存在性探针**经 PM 裁决 **保留、不立 `TD-004`**（与签名兼容垫片语义不同）→ **`BUG-003` / `BUG-004` 均置 `Verified`** → **④ 判达成 → `CHANGE-003` 收口（Closed）**。**已知边界已于 2026-09-14 消除：真实三方 AI 密钥已接入（`backend/.env`，不入库），real 通路已真实出网并成功**；残余风险 = 外部中转可用性（全站 502，见上方「真实三方 AI 联调已接入（2026-09-14）」））
...
- **`CR-006` 子项 A 开工（2026-09-16，PM）**：**`TD-005` 根治** —— `photos` 窗口归属冗余（`belong_date` / `group_key`，**上传时**解析，不再等挂接确认）。**实施前核验发现 CR 原假设不成立**：`ensure_group(session, family_id, *, student_id, category, belong_date)` 的 `belong_date` 是**入参**（非返回值）→ 无法据此得到归属日 → 改走 M001 **既有**契约接口 `TaskQueryService.resolve_window(ts) -> WindowInfo`（内部服务接口表**已列 M002 为消费者**；「纯计算、不锁配置」）→ **M001 零改动**。**契约修订已完成**：M002 **v0.4.2 → v0.4.3**（`MODULE_CONTRACT.md` / `MODULE_API.md`：`API-M002-002` 响应 +2 可选字段、`API-M002-003` +2 可选过滤参数、`PhotoDTO` 补 2 字段 / `MODULE_DATA.md`：`photos` +2 列 +2 索引 + 派生冗余规则 / `DATA_MODEL.md` DATA-003 / `API_REGISTRY.md` / `MODULE_CHANGELOG.md`（含 **v0.4.2 回溯补登**））；`MODULE_REGISTRY.md` M002 → v0.4.3。**签发**：`Task-022`（`AGENT-M002` 实施）/ `Task-023`（验收）。**子项 B 的 `input_ref` 承载性已核验可靠**（`app/core/ai/service.py:208` 明确写 `{"photo_id": …}`；存储为 JSON `TEXT`、**无独立索引** → 检索用 `json_extract`，必要时加表达式索引）。**迁移要点**：`main.py` 走 `Base.metadata.create_all`（**不为既有表加列**）→ V1 无生产数据，采用**重建库 + 重新种子**（`backend/data/*.db` 已 gitignore）。**待办**：执行 `Task-022` → `Task-023` 验收 → 子项 A 收口升版；随后子项 B / C。
- **`Task-022` 交付（`CR-006` 子项 A / `TD-005`，2026-09-16，PM 代执行）**：`photos` 窗口归属冗余（`belong_date`/`group_key`，**上传时**经 M001 契约内 `resolve_window` 解析）+ `API-M002-002` 响应 2 可选字段 + `API-M002-003` 2 可选过滤参数；新增 `tests/integration/test_photos_window_belong.py` **6 例**（含**周末合并**、**向后兼容哨兵**、**M001 不可用不阻断**、**单一知识源静态断言**）；**红→绿**成立（移除上传解析 → 4 例失败，`RED_EXIT=1`）；全量回归 **273 / 0 / 0 / 0**；**库重建迁移已执行并经真机复核**（`start_server.ps1 -Seed` → `Seed OK.`；真机 `201 / belong_date=2026-09-16 / group_key=2026-09-16`、**库内实读一致**、`task_id=None`（既有语义不变）、过滤 `1-0-1`）；`read_lints=0`；**`m001/**`、`core/ai/**` 零改动**。证据见 `docs/agents/Task-022.md §5`。**待办**：`Task-023` 验收 → 子项 A 收口升版。
- **新登记 `BUG-008`（中，`Confirmed`，2026-09-16，PM 读日志 + 读码确认属既有问题）**：**DATA-009（AI 调用记录）在 SQLite 锁竞争下静默丢失** —— 真实 AI 调用 `HTTP 200`，但 `record_call` 遇 `database is locked` → 仅 warning + `return None`（`app/core/ai/records.py:100-104`），**无重试 / 无计数 / 无补偿**，调用方无从得知「本次未留痕」；主链路不受影响（故级别**中**）。复现：`start_server.ps1 -Seed`（种子脚本与运行中服务并发写同库）。**只登记不修**；建议与 `CR-006` 子项 B（DATA-009 消费）同批评估。详见 `docs/changes/BUG-008.md`。
- **`CR-005` 落地收口（2026-09-16）**：四任务 `Task-018`（M002 受控取图）/ `Task-019`（M001 槽 + `API-M001-022`）/ `Task-020`（前端入口）/ `Task-021`（验收）全部完成，**8 条验收清单全通过** → `CR-005` 状态 **`Applied`**；`API-M001-022` → `Active`；`REQ-011` **第二阶段 A1 达成**（布置单图片 → 真实 Vision → 内容项）。证据原文见 `docs/agents/Task-021.md §5`。**另登记 `TD-006`**（M001 不校验图片源 `photo_id` 归属，防越权仅 M002 一道；无越权读取路径，风险低）。
- **真实三方 AI 联调已接入（2026-09-14，PM 执行）**：密钥源 = IDE `~/.codebuddy/models.json`（OpenAI 兼容中转），由一次性脚本读取后写入 `backend/.env`（**已 gitignore，不入库**）：`AT_AI_PROVIDER_MODE=real` + `AT_AI_ALLOW_MOCK_FALLBACK=false` + LLM `glm-5.3` / Vision·OCR `qwen3.8-flash`（`deepseek-v4-pro` 不支持 `json_object`，已排除）。**real 通路已真实出网并成功**：DATA-009 记录 `provider=openai_compatible / mock=0`；完整业务 prompt（569 字符 + `response_format=json_object`）→ **HTTP 200 + 合规 JSON**（`{"subjects":[{"subject":"math","contents":[{"text":"练习册 P23 第 1-10 题"}…]}`）。**端到端真实验收已通过（2026-09-14，中转限流恢复后复验）**：① **M001**：`POST /api/v1/tasks → 201 / spec_status=parsed`，真实解析产出 `math`+`chinese` 两条，DATA-009 `task_spec_parse / glm-5.3 / mock=0 / status=ok`；② **M002**：上传含真实文字的作业图 → `POST /api/v1/photos → 201`，照片 `status=suggested`、`links=[{subject:'math', source:'ai', confidence:1.0}]`，DATA-009 `photo_link_suggest / qwen3.8-flash / mock=0 / status=ok / 15.6s` → **真实 LLM + Vision 全链路打通并产出正确业务结果**。**注意：中转可用性仍间或波动**（本次捕获 1 条 `task_spec_parse` 约 20s 超时 `status=error`，重试后成功）→ 保留重试容错；此波动亦为 `BUG-006` 的现实触发条件（AI 间歇失败时 M001 会静默兜底成「假成功」）。配套基建：`backend/tests/conftest.py` 新增 autouse fixture **`isolate_deploy_config`**（关闭测试侧 dotenv 读取 + 清空 `AT_AI_*` 凭据 + 清 `get_ai_settings`/`get_ai_service`/`get_m002_settings` 三处 `lru_cache`）—— 因 `Settings`/`AISettings`/`M002Settings` 均 `env_file=".env"`，部署侧配置会污染回归（实测 8 例失败），隔离后全量回归恢复 **`EXIT=0`，基线 237 不减**。本次联调派生 **`BUG-005`**（错误分类与留痕失真）与 **`BUG-006`**（M001 静默 Mock 兜底）
- **AI 错误分类缺陷已确认（2026-09-14，`BUG-005`）**：三方 **403「余额/配额不足」被 `from_http_status` 误映射为 `auth_error`「三方鉴权失败」**（`app/core/ai/errors.py:67-68`），且 401/403/429/5xx 分支**丢弃三方响应体** → DATA-009 留痕失真、排障方向被误导（本次实测：真实原因 = 账户配额不足，记录却为「鉴权失败」）。级别 **中**；推荐新增 `AIErrorCode.QUOTA_EXHAUSTED`（**不可重试**）+ 三方响应摘要**脱敏截断**写入 `message`。详见 `docs/changes/BUG-005.md`
- **M001 解析静默兜底缺陷已确认（2026-09-14，`BUG-006`）**：`task_parser.default_parser` 在 AI 抛错/产物不合规后**无条件回落 `mock_parse_sources`**（`m001/services/task_parser.py:198-207`，全程**不读** `allow_mock_fallback`）→ 在 `real` + `AT_AI_ALLOW_MOCK_FALLBACK=false` 下 `POST /tasks` 仍返回 **`201 / spec_status="parsed"`**（草稿实为本地启发式），而同期 DATA-009 为 `mock=0 / status=error` —— **「假成功」且 UI 无降级信号**；对照组 M002 同配置行为正确（照片保持 `unassigned`、`links=[]`）。级别 **高**；与 `BUG-004` 同类（静默降级）但故障点不同（AI 可达、仅失败时不暴露）。详见 `docs/changes/BUG-006.md`
- **`Task-015` / `Task-016` 已签发（2026-09-14，PM）**：`Task-015`（**`AGENT-AI`**）修复 **`BUG-005`** —— 写区 = `backend/app/core/ai/errors.py`（唯一生产代码）+ 新增 `tests/unit/test_ai_errors.py`；内容 = 新增 `AIErrorCode.QUOTA_EXHAUSTED`（不可重试）、403 按配额线索（`quota`/`insufficient_quota`/`balance`/`pre_consume`/`余额`/`额度`）细分、三方响应摘要**脱敏 + ≤200 字符**入 `message`；要求 ≥8 例单测 + 红→绿判别力。`Task-016`（**`AGENT-M001`**）修复 **`BUG-006`** —— 写区 = `backend/app/modules/m001/services/task_parser.py`（仅 `default_parser` + 新增 `_mock_fallback_allowed()`）+ 单元/API 级用例；内容 = 降级判定服从 `provider_mode`（显式 `mock` 放行）与 `allow_mock_fallback`（`false` → 返回 `None` → 上层保持 `placeholder`），默认配置行为**不变**。两任务**写区零重叠、无依赖、可并行**。**修复已于 2026-09-14 完成（PM 代执行 —— 当前 IDE 无具备写权限的执行 subagent，仅有只读 `code-explorer`，故不构成独立第三方复核，此限制已如实标注于任务书 §5）**：`BUG-005` / `BUG-006` 状态置 **`Fixed`**；全量回归 **251 / 0 failed / 0 errors / 0 skipped**（基线 237 + 新增 14 例，`GREEN_EXIT=0`）；两处**红→绿判别力成立**（`Task-016` 含 **API 面**证据：禁兜底 → `placeholder`，回退 → `parsed`「假成功」复现）；写区双证（`git status` + mtime）+ `read_lints=0` 齐备。`Task-016` 另行使**写区最小扩权**（`tests/e2e/test_acceptance_scenarios.py::test_scenario6_*` **仅后移 env 切换时机**、断言逐字不变，见 `Task-016 §3.6`）。证据原文见 `docs/agents/Task-015.md §5` / `Task-016.md §5`；待办 = 统一入库 + `docs/DATA_MODEL.md` 登记 `quota_exhausted` + `CHANGELOG` Release Note（`spec_status` 行为面变更）。
- **`REQ-011` / `Task-017` / `BUG-007` / `TD-005` 已登记（2026-09-14，PM；需求经 8 轮 grill 定稿）**：**`REQ-011`（Approved）** = 任务详情页「**AI 未识别的照片**」**只读卡片 + 单张重试** —— **第一阶段（A2，本期）**：**零后端 / 零契约**（复用 `API-M002-003` + `API-M002-007`），口径 = **该学生 `kind=homework` + `status=unassigned`** 照片（**非**本任务窗口，因未挂接照片无窗口归属，见 `TD-005`），卡片置页面最底部、只读 + 单张「重试建议」，**重试请求单独放宽 timeout 至 60s**（全局 15s 不动）；成败按响应 `suggestions` 空/非空反馈（成功 → 去作业页采纳；失败 → 提示可能暂不可用 + 手工挂接）；**第二阶段（A1）** = 布置单图片 → AI 解析内容项，**需 CR/ACR** 后另立。**`Task-017` 已签发**（`AGENT-M001`，前端增量；写区 = `TaskDetailView.vue` + `api/index.ts`（**最小扩权**：仅 `getLinkSuggestions` 增加可选 `timeoutMs`）；`backend/**`、`PhotoCard.vue`、`http.ts`、`router/**` **零写入**）。**`BUG-007`（中-高，Confirmed）** = 全局 `timeout=15000` vs `API-M002-007` **同步** AI 调用实测 **15630ms** → 现有「重试建议」在真实模型下**必然前端超时（成功却报失败）**，Mock 毫秒级掩盖了该缺陷；由 `Task-017` 止血修复，根治需 `API-M002-007` 异步化（CR 候选）。**`TD-005`（新增）** = 未挂接照片**无窗口归属**（`photos.task_id` 仅首条挂接确认时写入）→ 无法按窗口过滤/展示。**后续候选（未立项）**：「AI 失败原因对用户可见」（消费 `DATA-009.error.code`）、`API-M002-007` 异步化、A1 第二阶段。**第一阶段（A2）已完成（`Task-017`，2026-09-14，PM 代执行）**：任务详情页「AI 未识别的照片」只读卡片 + 单张重试已落地（`getLinkSuggestions` 新增可选 `timeoutMs`，**全局 15s 未动**）；`vue-tsc -p tsconfig.app.json` **0 error**、`npm run build` **EXIT=0**、**浏览器级失败分支 8/8 + 成功分支 6/6（真实三方 AI：toast「已生成挂接建议」+ 照片移出列表）**、后端全量 **251 / 0 / 0 / 0（用例数不变 → 零影响）**；**`BUG-007` → `Fixed`**（含 `Task-017 §3.3b` **最小扩权**：`PhotoListView.vue` 仅 1 行调用放宽单请求超时，使作业页同一失效路径一并闭环）。
- **`CR-005` / `CR-006` 提案登记（2026-09-14，PM；**状态 `Proposed`，待用户批准**）**：按 `REQ-011` 后续与推荐顺序，两项待办**均为契约变更** → 依铁律先登记 CR、**不动契约与代码**。
  - **`CR-005`（布置单图片源解析通路 + 重新解析端点）** = `REQ-011` 第二阶段 A1：M001 新增「布置单图片读取」回调槽（`register_task_spec_image_provider`，与 `register_links_migration_hook` 同模式）+ `_to_ai_sources` 扩展 + **新端点 `API-M001-022 POST /tasks/{task_id}/reparse`**（`confirmed` → `409`；AI 失败**不抛错**、保持 `placeholder`，沿用 `BUG-006` 语义；§2.4 内容项覆盖规则：`placeholder` 写入 / `parsed` 整体替换 / 失败**不清空**）；M002 新增受控 `provide_task_source_image`（归属校验、**路径不外发**）；契约 M001 **v0.2.0→v0.3.0**、M002 **v0.4.1→v0.4.2**。**`app/core/ai` 零改动**（`ImageInput(path|data)` + `SourceInput.image_of` + Vision 通路**均已具备**，且已有单测覆盖）。**无需 ACR/ADR**（不改数据所有权 —— 仍由 Owner M002 提供受控接口；不改判定模型；沿用既有跨模块回调通道）。**硬约束**：**仅真实 Vision（非 Mock、非 degraded）才转发图片源** —— Mock 会产出占位草稿（`BUG-004` 教训）。批准后拆 `Task-018`（M002 provider）→ `Task-019`（M001 消费）→ `Task-020`（前端）→ `Task-021`（验收）。
  - **`CR-006`（M002 照片域三项增强，可分子项批准）** = **A** `TD-005`：`photos` 新增 `belong_date`/`group_key` 冗余列（**上传时**解析落库）+ `GET /photos` 新增窗口过滤参数 → `REQ-011` 卡片可由「学生级」升级为「本任务窗口」口径｜**B** AI 失败原因对用户可见：`link-suggestions` 响应新增可选 `last_attempt`（读 DATA-009，继承 `BUG-005` 的脱敏 + 截断约束；**实施前须核验 `input_ref` 是否可靠承载 `photo_id`**）｜**C** retry **异步化根治**（新增 `POST /photos/{id}/link-suggestions/retry` → `202` + 前端轮询；**保留**同步路径兼容）。建议优先级 **A → B → C**。
- **`CR-005` / `CR-006` 已批准并进入执行（2026-09-16，用户批准）**：**契约修订已完成** —— M001 **v0.2.0 → v0.3.0**（新增 **`API-M001-022` `POST /tasks/{task_id}/reparse`**（`Draft`，随实施转 Active）+ 内部回调槽「布置单图片源读取」）、M002 **v0.4.1 → v0.4.2**（内部服务接口 +1：`provide_task_source_image`）；`API_REGISTRY.md` / `MODULE_REGISTRY.md` 已同步。**已签发 4 份任务书**：`Task-018`（`AGENT-M002`：受控提供图片 + 幂等注册/自愈）、`Task-019`（`AGENT-M001`：回调槽 + `_to_ai_sources` 图片源转发 + `API-M001-022`，含**防伪造硬约束** —— 仅真实 Vision 才转发）、`Task-020`（前端「重新解析」入口）、`Task-021`（跨模块真机 + 浏览器级验收，只读）。**`CR-006` A/B/C 已批准、待排期**（建议顺序 A → B → C，须在 `CR-005` 任务链之后串行以避免同文件冲突）。
