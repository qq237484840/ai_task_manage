...
## TD-006 —— M001 不校验图片源 `photo_id` 归属（防越权仅 M002 一道）

| 项 | 内容 |
| --- | --- |
| 来源 | `CR-005` 验收真机取证（`Task-021`，2026-09-16） |
| 位置 | `backend/app/modules/m001/services/task_parser.py::_to_ai_sources`（图片源仅透传 `photo_id` 给 M002 回调槽）；`m001/services/task_service.py` 的建任务 / `reparse` 路径**无** `photo_id` 归属预校验 |
| 现状 | 实测**家庭 B 可建出引用家庭 A 照片**的任务（`POST /tasks` → `201`，M001 不报错）；防越权**完全依赖** M002 `provide_task_source_image` 的 `family_id` 过滤（越权 → `None` → 不转发图片源）。实测该防线有效：跨家庭 `reparse` → `placeholder` + `contents==[]`，**无内容或字节外泄** |
| 影响 | **当前无越权读取路径**（M001 仅透传 `photo_id` 字符串；任务详情页只显示 ID，不渲染图片字节）→ 风险**低**。但「单点防线」结构脆弱：若新增图片源消费面（缩略图渲染 / 导出 / 第三方回调），或 M002 归属过滤被改坏，将直接产生**跨家庭越权取图** |
| 触发条件 | 新增任何「按 `photo_id` 读取或展示图片字节」的功能；或 M002 归属过滤逻辑变更 |
| 建议方案 | ① **推荐**：M001 在 `ingest` / `reparse` 落源前做**归属预校验**（M002 暴露轻量「归属只读校验」接口，或给现有回调槽加「只校验不取图」语义）→ 形成**纵深防御**（两道）；② 或**显式记录**「图片源归属由 M002 单一防线保证」为设计决定（并在 M002 侧保留契约级归属用例）；③ 最轻：为 M002 归属过滤加「冻结 + 变更走 CR」标注 |
| 责任 / 时机 | Project Master（是否走 CR 决策）+ `AGENT-M001`/`AGENT-M002`；**出现图片字节消费面时必须处理** |
...
- **~~第二阶段需求（A1）：布置单图片 → AI 解析内容项~~ —— 已完成（`CR-005`，2026-09-16，`Applied`）**：M001 经 M002 **受控取图**（沿用既有唯一回调通道，非直连）转发图片源 → 真实 Vision 解析出内容项；新增 `API-M001-022`（`POST /tasks/{task_id}/reparse`）。真机证据（真实 Vision `parsed` + 4 项内容 + DATA-009 `mock=0/ok`）见 `docs/agents/Task-021.md §5`。
