...
- **任务队列**：`Task-006`~`Task-021` 均已完成，**无 Active**。**`CR-005` 已 `Applied`（2026-09-16）**：`Task-018`（M002 受控取图）/ `Task-019`（M001 回调槽 + `API-M001-022`）/ `Task-020`（前端「重新解析」入口）/ `Task-021`（验收，8 条全通过）→ **`REQ-011` 第二阶段 A1 达成**（布置单图片 → 真实 Vision → 内容项）。**`CR-006` 子项 A（2026-09-16）**：契约修订完成（M002 **v0.4.2 → v0.4.3**）+ **`Task-022` 已交付**（PM 代执行；全量 **273/0/0/0** + 红→绿 + **库重建真机复核**）→ **待 `Task-023` 验收**；子项 B/C 待排期（`BUG-008` 建议与子项 B 同批）。**用户口径（2026-09-16）：「密钥暂不轮换」**（事件记录见下）。其余候选 = `TD-006`（`TD-005` 由子项 A 根治）、CR 候选（失败原因可见 = 子项 B；`API-M002-007` 异步化 = 子项 C）、`TD-001`/`TD-002`，或 **V2 域 D（M005~M007，`RISK-012`）**。
...
- **venv** = `backend/.venv`（Python 3.12）；**pytest 基线 = 273**（2026-09-16；沿革 237 → 251（`Task-015`/`016`）→ 257（`Task-018`）→ 265（`Task-019`）→ 267（`Task-021` +2）→ **273**（`Task-022` +6））；端口统一 **8010**（8000 被 `wslrelay.exe` 常占）。
