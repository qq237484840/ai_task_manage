...
- **缺陷台账**：`BUG-001`~`BUG-004` 均 **Verified** ｜ **`BUG-005` / `BUG-006` = `Fixed`（2026-09-14，`Task-015`/`Task-016` 修复；由 **PM 代执行** —— 本机无具备写权限的执行 subagent，不构成独立第三方复核，限制已标注任务书 §5）**：全量回归 **251/0/0/0** + 双**红→绿**（`BUG-006` 含 **API 面**：禁兜底→`placeholder`，回退→`parsed` 复现）+ 写区双证 + `read_lints=0` ｜ **`BUG-007` = `Fixed`（2026-09-14，`Task-017`；含 §3.3b 最小扩权使作业页同一超时路径闭环）** ｜ **`BUG-008` = `Confirmed`（2026-09-16 新登记，中：DATA-009 在 SQLite 锁竞争下静默丢失 —— 真实调用 `200` 但记录写入 `database is locked` → 仅 warning 丢弃，`records.py:100-104`，无重试/无计数；主链路不受影响；只登记不修，建议与 `CR-006` 子项 B 同批）** ｜ **`BUG-009` = `Confirmed`（2026-09-17 新登记，**高**：**DATA-009 写失败（SQLite 锁竞争）→ Session 中毒 → `API-M002-007` 返回 500** —— `app/core/ai/records.py:100-104` 的 `except` **只 warning、不 `session.rollback()`** → Session 进「待回滚」→ `m002/api/link_routes.py:88` 同 Session `commit()` 抛 `PendingRollbackError`；触发 = 真机**后台建议线程 × `retry`** 并发写；**持写锁确定性复现 + 基线对照（`git stash`）证明非子项 B 引入**；**修正 `BUG-008` 「主链路不受影响」结论**；只登记不修，**建议与 `BUG-008` 合并修复**（同源、同写区），优先级置于子项 C 前）**。
...
- **任务队列**：`Task-006`~`Task-021` 均已完成，**无 Active**。**`CR-005` 已 `Applied`（2026-09-16）**：`Task-018`（M002 受控取图）/ `Task-019`（M001 回调槽 + `API-M001-022`）/ `Task-020`（前端「重新解析」入口）/ `Task-021`（验收，8 条全通过）→ **`REQ-011` 第二阶段 A1 达成**（布置单图片 → 真实 Vision → 内容项）。**`CR-006` 子项 A 已收口（2026-09-17，`v0.25.0`，`TD-005` 根治）**：契约修订（M002 **v0.4.2 → v0.4.3**）+ `Task-022` 交付 + **`Task-023` 验收 8 条全通过**（真机 13/13、全量 **274/0/0/0**、红→绿、生产代码零写入）；**验收补强 1 处覆盖缺口**（原「不阻断」用例替换了被测容错层 → 拆双分支）。**子项 B 已收口（2026-09-17，`v0.26.0`）**：契约修订 M002 **v0.4.3 → v0.4.4**（`API-M002-007` 响应 +`last_attempt`，消费 DATA-009；**版本校正教训**：子项 A 已占用 v0.4.3 → 子项 B 必为 v0.4.4，勿与 A 撞版）+ `Task-024` 交付（**首版提交 `1afb33c` 有 5 处硬缺口：检索键用了**不存在的** `photo.upload_request_id` → 功能恒 `null`**；已全部修复）+ `Task-025` 验收 8 条全通过（真机 **11/11**、全量 **278/0/0/0**、两组红→绿、生产代码零写入）；**下一步 = 子项 C**（重试异步化，实施前须核验既有异步调度形态；**建议先修 `BUG-008`+`BUG-009` 合并项**）；另 `REQ-011` R1 前端口径升级（学生级 → **本任务窗口**，现由子项 A 支撑）需另立前端任务。**用户口径（2026-09-16）：「密钥暂不轮换」**（事件记录见下）。其余候选 = `TD-006`（`TD-005` 由子项 A 根治）、CR 候选（失败原因可见 = 子项 B；`API-M002-007` 异步化 = 子项 C）、`TD-001`/`TD-002`，或 **V2 域 D（M005~M007，`RISK-012`）**。
...
- **`CR-006`（M002 照片域三项增强，用户批准 2026-09-16 A/B/C 全批）**：**子项 A = `TD-005` 窗口归属**（`photos` 加冗余列 **`belong_date`/`group_key`**，**上传时**经 M001 **既有**契约接口 `TaskQueryService.resolve_window(ts) -> WindowInfo{belong_date, week_index, window_type, group_key}` 解析 —— **M001 零改动**；**关键教训：CR 原设想「复用 `ensure_group` 返回窗口上下文」不成立**，因 `belong_date` 是它的**入参**；迁移 = **重建库**（`main.py` 用 `create_all`，不给既有表加列））；**子项 B = AI 失败原因可见**（`API-M002-007` 响应 +`last_attempt`，读 DATA-009；**`input_ref` 已核验可靠承载 `photo_id`**（`service.py:208`），但为 JSON `TEXT` **无索引** → 检索用 `json_extract`）；**子项 C = 重试异步化**（`POST /photos/{id}/link-suggestions/retry` → `202` + 轮询）。实施须**串行 A → B → C**。
...
- **venv** = `backend/.venv`（Python 3.12）；**pytest 基线 = 278**（2026-09-17；沿革 237 → 251（`Task-015`/`016`）→ 257（`Task-018`）→ 265（`Task-019`）→ 267（`Task-021`）→ 273（`Task-022` +6）→ 274（`Task-023` 验收补强 +1）→ **278**（`Task-024` +4））；端口统一 **8010**（8000 被 `wslrelay.exe` 常占）。
