...
- 状态 **v0.11.0**：Phase 3 功能域主线 + 内容级重构 —— **M001 Stable v0.1.2**（Task-001 APPROVED + **CHANGE-001 Applied**：CR-001/CR-002/ACR-001/ACR-002 PM 复核 APPROVED 2026-09-08，API-M001-013~017 Active，维护态）；**M002 Developing（契约 v0.3.0 Frozen，Task-002 已签发 `docs/agents/Task-002.md`，AGENT-M002 编码中）**；主线计划=`ROADMAP.md`（当前=采集链 M-A）。REQ-001~009 Approved；PD-001~024 全部确认；跨模块开放项 O-1~O-9 见 PROJECT_STATUS（O-8 已关闭）。
...
- 当前阶段：Phase 3 —— **M001 Stable v0.1.2（维护态）+ M002 编码中（Task-002）**：M001 工程与 **89** 测试细节见 `docs/modules/M001/`（student_accounts/student_auth 等新文件已登记）；**M002（作业图片采集与归属）契约 v0.3.0 Frozen 落库 `docs/modules/M002/`**：先采后认归属保留 + 完成程度改由内容级判定链回写（R4~R6），API-M002-001~006（Frozen）；图片存 `<backend>/data/images` 分层、鉴权读取+访问审计；**主线按功能域重排见 `docs/ROADMAP.md`**；**待办（下一开发段）**：AGENT-M002 按任务书 `docs/agents/Task-002.md` 实现 M002 v0.3.0（backend modules/m002 + 前端基础 UI + tests + 回填；M001 依赖 get_task_group/can_accept_photo/mark_in_progress/双主体认证已就绪；M003 建议写入口预留、AI 建议触发留 M003 契约轮）→ PM 按 ROADMAP M-A 里程碑 DoD 验收 → M003~M007 依序（M-A~M-D）→ 全系统回归（真实三方+Mock 双跑）。细节见 `docs/modules/M002/MODULE_*.md`、`docs/agents/Task-002.md`、`docs/ROADMAP.md` 与 `.codebuddy/memory/2026-09-08.md`。
